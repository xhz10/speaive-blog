...
public final class MarkdownInboxImporter {
    ...
    public MarkdownInboxImporter(
        ...
        this(importDirectory, maxMarkdownBytes, contentStore, mapper, transactions, Clock.systemUTC());
    }
    ...
    private void process(Path file) {
        ...
        try {
            ...
            String sha256 = PostgresContentStore.sha256(bytes);
            ImportDecision decision = transactions.execute(status -> {
                if (mapper.countImport(sha256) > 0) {
                    return ImportDecision.ALREADY_IMPORTED;
                }
                Post post = contentStore.importDraft(fileName, bytes);
                mapper.insertImport(sha256, fileName, post.slug(), clock.instant());
            ...
            log.info("Markdown inbox {}: {}", decision == ImportDecision.IMPORTED ? "imported" : "deduplicated",
                    fileName);
        } catch (ImportFailure exception) {
            reject(file, exception.getMessage());
        } catch (BlogException exception) {
            if (exception.code() == com.speaive.blog.application.BlogErrorCode.STORAGE_ERROR) {
