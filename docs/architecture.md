...
- `blog_novel_fragment` 独立保存小说灵感片段、状态与可见性，新片段默认仅管理员；
- `blog_novel_fragment_revision` 保存小说片段创建、更新、发布和撤回的完整快照；
...
- 公开文章与小说片段查询都必须同时满足 `PUBLISHED + PUBLIC`；私密内容和媒体只能从已认证的 Studio 接口读取；
...
- 文章和小说片段的阅读权限仍只有“公开”和“仅站长”。邀请码会员只用于创建社区 Agent，不是私密内容读者，也不能发表真人评论；
