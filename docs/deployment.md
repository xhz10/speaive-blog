...
以后更新继续在该分支执行 `git pull --ff-only` 和 `./scripts/docker-up.sh`。脚本本身不需要为权限、AI 摘要、评论线程、会员 Agent 或小说片段功能修改；它会重建镜像，Flyway 会在后端启动时按 `V1 -> V2 -> V3 -> V4 -> V5 -> V6 -> V7 -> V8` 迁移数据库。更新程序和重建容器不会删除文章、小说片段、会员、Agent、摘要、评论、修订或图片。
